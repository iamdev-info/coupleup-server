...
// To up automatically add the createdAt if not present
  ...
  return new Promise((resolve, reject) => {
    this.find({})
      ...
      .exec((err: any, docs: any) => {
        if (err)
          return reject(
            Boom.internal(getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR))
          );
        return resolve(docs);
      });
  });
...
userSchema.statics.fetchByID = function (userID: string) {
  return new Promise((resolve, reject) => {
    this.findById(userID)
      .select("-__v -password")
      .exec((err: any, user: IUserDocument) => {
        if (err)
          return reject(
            Boom.internal(getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR))
          );
        if (!user)
          return reject(Boom.notFound(getReasonPhrase(StatusCodes.NOT_FOUND)));
        return resolve(user);
      });
  });
};
...
userSchema.statics.insertOne = function (newUser: any) {
  return new Promise((resolve, reject) => {
    this.create(newUser, (err: any, user: IUserDocument) => {
      if (err) {
        if (err.code === 11000)
          // return reject(Boom.badRequest(EMAIL_ALREADY_TAKEN));
          return reject(
            Boom.internal(getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR))
          );
      }
...
userSchema.statics.modifyByID = function (userID: string, modifications: {}) {
  return new Promise((resolve, reject) => {
    this.findOneAndUpdate({ _id: userID }, modifications, { new: true });
  });
};
...
userSchema.statics.deleteByID = function (userID: string) {
  return new Promise((resolve, reject) => {
    this.prototype.findOneAndRemove(userID, (err: any, user: IUserDocument) => {
      if (err)
        return reject(
          Boom.internal(getReasonPhrase(StatusCodes.INTERNAL_SERVER_ERROR))
        );
